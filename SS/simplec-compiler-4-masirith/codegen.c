...
static void codegen_assignstmt(T_stmt stmt) {
  // if else to separate the two cases of assign and assign with dereference

  // assign
  if (E_op_deref != stmt->assignstmt.left->unaryexpr.op) {
    // generate code for the right-hand side of the assignment
    codegen_expr(stmt->assignstmt.right);
    // pop the right-hand side into %rax
    POP("%rax");
    // store the right-hand side in the left-hand side
    int offset = lookup_offset_in_scope(current_offset_scope, stmt->assignstmt.left->identexpr);
    MOV_TO_OFFSET("%rax", offset);
  }
  // assign with dereference
  else {
    ...
    // pop the right-hand side into %rax
    POP("%rax");
    // pop the left-hand side into %rbx
    POP("%rbx");
    // store the right-hand side in the left-hand side
    MOV("%rax", "(%rbx)");
  }
...
static void codegen_ifstmt(T_stmt stmt) {
  ...
  // generate the label for the else branch
  LABEL(labelnum);
  labelnum++;
  // fprintf(stderr, "TODO: codegen_ifstmt (remove this message when implemented).");
}
...
static void codegen_ifelsestmt(T_stmt stmt) {
  ...
  int endlabelnum = labelnum + 1;
  JMP(endlabelnum);
  // generate the label for the else branch
  LABEL(labelnum);
  labelnum++;
  // generate code for the body of the else statement
  codegen_stmt(stmt->ifelsestmt.elsebranch);
  // generate the label for the end of the if-else statement
  LABEL(endlabelnum);
  labelnum++;
  // fprintf(stderr, "TODO: codegen_ifelsestmt (remove this message when implemented).");
}
...
static void codegen_whilestmt(T_stmt stmt) {
  ...
  LABEL(endlabelnum);
  labelnum++;
  // fprintf(stderr, "TODO: codegen_whilestmt (remove this message when implemented).");
}
...
static void codegen_unaryexpr(T_expr expr) {
  ...
  else if (E_op_ref == expr->unaryexpr.op) {
    // only allow an identexpr to be referenced
    if (E_identexpr != expr->unaryexpr.expr->kind) {
      fprintf(stderr, "FATAL: unexpected kind in codegen_unaryexpr\n");
      exit(1);
    }
    // save base pointer to another register
    MOV("%rbp", "%rax");
    // get the offset of the identexpr
    int offset = lookup_offset_in_scope(current_offset_scope, expr->unaryexpr.expr->identexpr);
    // compute address by subtracting offset from base pointer copy in rax
    SUBCONST(offset, "%rax");
  ...
  // unary dereference operator *
    ...
    fprintf(stderr, "FATAL: unexpected unary operator in codegen_unaryexpr\n");
    exit(1);
  }
...
static void codegen_binaryexpr(T_expr expr) {
  ...
  case E_op_mod:
    ...
    break;
  case E_op_eq:
    // COMMENT("do the equality comparison");
    // CMP("%rbx", "%rax");
    // MOV("$0", "%rax");
    // JE(labelnum);
    // MOV("$1", "%rax");
    ...
    break;
  case E_op_ne:
    // COMMENT("do the inequality comparison");
    // CMP("%rbx", "%rax");
    // MOV("$0", "%rax");
    // JNE(labelnum);
    // MOV("$1", "%rax");
    ...
    break;
  case E_op_lt:
    COMMENT("do the less-than comparison");
    CMP("%rbx", "%rax");
    JL(labelnum);
    MOV("$0", "%rax");
    JMP(labelnum);
    ...
    break;
  case E_op_le:
    COMMENT("do the less-than-or-equal comparison");
    CMP("%rbx", "%rax");
    MOV("$0", "%rax");
    JLE(labelnum);
    MOV("$1", "%rax");
    ...
    break;
  case E_op_gt:
    COMMENT("do the greater-than comparison");
    CMP("%rbx", "%rax");
    JG(labelnum);
    MOV("$0", "%rax");
    JMP(labelnum);
    ...
    break;
  case E_op_ge:
    COMMENT("do the greater-than-or-equal comparison");
    CMP("%rbx", "%rax");
    MOV("$0", "%rax");
    JGE(labelnum);
    MOV("$1", "%rax");
